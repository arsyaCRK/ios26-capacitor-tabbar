...
export interface RectMetrics { x: number; y: number; width: number; height: number }
export interface EdgeInsetsMetrics { top: number; left: number; bottom: number; right: number }
...
export interface TabBarPlugin {
  ...
  setLongPressEnabled(options: { enabled: boolean }): Promise<void>; getTabBarMetrics(): Promise<TabBarMetrics>; lockTabBar(): Promise<void>; unlockTabBar(): Promise<void>;
  ...
  addListener(eventName: 'tabBarMetrics', listenerFunc: (data: TabBarMetrics) => void): Promise<{ remove: () => void }>;
}
